...
void compileTypeDecls(void) {
  // TypeDecls ::= TypeDecl ; TypeDecls | epsilon
  if (lookAhead->tokenType == TK_IDENT) {
    compileTypeDecl();
    compileTypeDecls();
  }
...
void compileType(void) {
  ...
  // Type ::= KW_CHAR
  // Type ::= TypeIdent
  // Type ::= KW_ARRAY SB_LSEL Number SB_RSEL KW_OF Type
  switch (lookAhead->tokenType) {
  case KW_INTEGER:
  case KW_CHAR:
    compileBasicType();
...
void compileAssignSt(void) {
  ...
  // AssignSt ::= FunctionIdent SB_ASSIGN Expression
  eat(TK_IDENT);
...
void compileCondition2(void) {
  // Condition2 ::= SB_EQ | SB_NEQ | SB_LE | SB_LT | SB_GE | SB_GT Expression
  switch (lookAhead->tokenType) {
  case SB_EQ:
    eat(SB_EQ);
    compileExpression();
